  ...
  public GraphicalInterface() {
    ...
    btnLoadTaxpayer.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        String[] fileSplitted = file.getAbsolutePath().split("\\\\");

        String fileName = fileSplitted[fileSplitted.length -1];
        String taxRegistrationNumber = fileName.split("_")[0];

        try {
          if (taxpayerManager.containsTaxpayer(Integer.parseInt(taxRegistrationNumber))) {
      ...
      public void mousePressed(MouseEvent e) {
        if (e.getClickCount() == 2) {
          ...
          try {
            ...
            JOptionPane.showMessageDialog(null, "You must select a tax registation number.");
          } catch (Exception e1) {
            e1.printStackTrace();
          }
        }//end if
