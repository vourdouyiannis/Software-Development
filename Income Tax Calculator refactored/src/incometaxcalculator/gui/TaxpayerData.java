...
public class TaxpayerData extends JFrame {
  ...
  public TaxpayerData(int taxRegistrationNumber, TaxpayerManager taxpayerManager) {
    ...
    receiptsList.addMouseListener(new MouseAdapter() {
      public void mousePressed(MouseEvent e) {
        if (e.getClickCount() == 2) {
          int receiptID = receiptsList.getSelectedValue();
          String receiptString = taxpayerManager.getTaxpayer(taxRegistrationNumber).getReceiptHashMap().get(receiptID).toString();
          JOptionPane pane = new JOptionPane(receiptString);
          JDialog dialog = pane.createDialog("Receipt for \'" + taxRegistrationNumber + "\' taxpayer");
    ...
    btnAddReceipt.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        receiptImporterPanel.add(new JLabel("Date: (dd/MM/yyyy)"));
        receiptImporterPanel.add(date);

        receiptImporterPanel.add(new JLabel("Kind:"));
        JComboBox<String> receiptKindMenu = new JComboBox<>(receiptKinds);
        receiptImporterPanel.add(receiptKindMenu);
        ...
        if (op == 0) {
          ...
          kindValue = receiptKindMenu.getSelectedItem().toString();
    ...
    btnDeleteReceipt.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        int receiptIDValue = -1;
        try {
          if(receiptsList.getSelectedValue() == null) {
          ...
          JOptionPane.showMessageDialog(null, "Problem with opening file ." + taxRegistrationNumber + "_INFO.txt");
        } catch (WrongReceiptKindException e1) {
          JOptionPane.showMessageDialog(null, "Please check receipt's kind and try again.");
        }
        catch (NumberFormatException e1) {
          JOptionPane.showMessageDialog(null, "You must select a receipt ID.");
        }
      }
    });
